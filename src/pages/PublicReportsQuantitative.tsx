...
interface PublicQuantitativeSummary {
  fiscal_years: number[];
  region_passed_all_17: {
    health_region_id: string;
    passed_all_17: number;
  }[];
  province_passed_all_17: {
    province_id: string;
    passed_all_17: number;
  }[];
  province_avg_quantitative_score: {
    province_id: string;
    avg_quantitative_score: number | null;
  }[];
}
...
export default function PublicReportsQuantitative() {
  ...
  useEffect(() => {
    const fetchBaseData = async () => {
      try {
        setBaseLoading(true);
        const [regionsRes, provincesRes, hospitalsRes, healthOfficesRes] = await Promise.all([supabase.from('health_regions').select('*').order('region_number'), supabase.from('provinces').select('*').order('name'), supabase.from('hospitals').select('*').order('name'), supabase.from('health_offices').select('*').order('name')]);
    ...
    const fetchSummary = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.rpc('get_public_quantitative_summary', {
  ...
  return <PublicLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <button onClick={handleBackToRegions} className={`hover:text-primary ${selectedRegion === 'all' ? 'font-medium text-foreground' : ''}`}>
            ทุกเขตสุขภาพ
          </button>
          {selectedRegion !== 'all' && <>
              <span>/</span>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="text-center py-8 text-muted-foreground">กำลังโหลด...</div> : selectedRegion === 'all' ?
          // Region level table
          <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {regionTableData.map(row => <TableRow key={row.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleRegionClick(row.id)}>
                        ...
                        const colorClass = percentage === 100 ? '[&>div]:bg-success' : percentage >= 50 ? '[&>div]:bg-warning' : '[&>div]:bg-destructive';
          ...
          <div className="overflow-x-auto">
                <button onClick={handleBackToRegions} className="mb-4 text-sm text-primary hover:underline flex items-center gap-1">
                  ← กลับไปยังเขตสุขภาพ
                </button>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {provinceTableData.map(row => <TableRow key={row.id}>
