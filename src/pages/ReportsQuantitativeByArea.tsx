...
export default function ReportsQuantitativeByArea() {
  ...
  const calculateCategoryAverages = (
    hospitalIds: string[],
    healthOfficeIds: string[] = [],
    isProvinceLevel: boolean = false,
  ) => {
  ...
  const { filteredTableData, safetyCounts } = useMemo(() => {
    ...
    const dataWithLevel = tableData.map((row) => {
      ...
      if (percentage === 100) {
        green++;
        return { ...row, safetyLevel: "green" as const };
      }
      if (percentage >= 50) {
        yellow++;
        return { ...row, safetyLevel: "yellow" as const };
      }
      red++;
    ...
    const filtered =
      selectedSafetyFilter === "all"
        ? dataWithLevel
        : dataWithLevel.filter((r) => r.safetyLevel === selectedSafetyFilter);
  ...
  const handleExportExcel = () => {
    ...
    const rows = filteredTableData.map((row) => {
      ...
      if (
        (row.type === "province" || row.type === "region") &&
        "hospitalsPassedAll17" in row &&
        "hospitalCount" in row
      ) {
      ...
      row.categoryAverages.forEach((catAvg) => {
        ...
        if (row.type === "region" || row.type === "province") {
          ...
          rowData.push(
            catAvg.average === 1 ? "ผ่าน" : catAvg.average === 0 ? "ไม่ผ่าน" : Number(catAvg.average.toFixed(2)),
          );
        }
      });
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">คะแนน 17 ข้อ แยกตามหน่วยงาน</h1>
              <p className="text-muted-foreground">
                แสดงคะแนนเฉลี่ยเชิงปริมาณ (17 ข้อ CTAM) แยกตามเขตสุขภาพ จังหวัด และหน่วยบริการ
              </p>
            </div>
          </div>
          ...
          <CardContent>
            {(() => {
              ...
              const left = {
                ...
                percentGreen:
                  sticky.name +
                  (showSummaryCols ? sticky.hospitalCount + sticky.hospitalsAssessed + sticky.passedAll17 : 0),
              } as const;
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {[
                        ...
                        },
                        {
                          key: "notSubmitted",
                          label: "ยังไม่ประเมิน",
                          count: safetyCounts.notSubmitted,
                      ...
                      <Table className="min-w-max">
                        <TableHeader>
                          <TableRow className="bg-muted/50">
                            ...
                            {showSummaryCols && (
                              <TableHead
                                className={`${stickyHeaderBase} text-center min-w-[100px] bg-blue-100 dark:bg-blue-900/30`}
                                style={{ left: left.hospitalsAssessed }}
                              >
                            ...
                            <TableHead
                              className={`${stickyHeaderBase} text-center min-w-[200px] bg-primary/10`}
                          ...
                          {filteredTableData.map((row) => {
                            ...
                            return (
                              <TableRow key={row.id} className="hover:bg-muted/30">
                                ...
                                {showSummaryCols && (
                                  <TableCell
                                    className={`${stickyCellBase} text-center font-medium bg-blue-50 dark:bg-blue-900/20`}
                                    style={{ left: left.hospitalsAssessed, minWidth: sticky.hospitalsAssessed }}
                                  >
                                    {"hospitalsAssessed" in row ? row.hospitalsAssessed : 0}
                                  </TableCell>
                                )}
                                ...
                                <TableCell
                                  className={`${stickyCellBase} text-center bg-primary/5`}
                                  style={{ left: left.percentGreen, minWidth: 200 }}
                                >
                                  {(() => {
                                    let percentage: number;
                                    if (
                                    ...
                                    if (isUnitLevel) {
                                      ...
                                      if (latestAssessment) {
                                        const latestAssessmentItems = filteredAssessmentItems.filter(
                                          (item) => item.assessment_id === latestAssessment.id,
                                        );
                                        unitPassedCount = latestAssessmentItems.filter(
