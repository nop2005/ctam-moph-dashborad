...
const SCORE_RANGES: { s: number; r: string }[] = [
  { s: 10, r: '≥ 80%' },
  { s: 9, r: '75% - 79.99%' },
  { s: 8, r: '70% - 74.99%' },
  { s: 7, r: '65% - 69.99%' },
...
function ScoreRangePopover({ label }: { label: string }) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button type="button" className="inline-flex items-center gap-1 mx-auto hover:underline cursor-pointer">
          <span>{label}</span>
                  ...
                  <Table className="min-w-max">
                    <TableHeader>
                      <TableRow className="bg-muted/50">
                        ...
                        {showSummaryCols && <TableHead colSpan={3} className={`${stickyHeaderBase} text-center bg-amber-100 dark:bg-amber-900/30`} style={{
                        left: left.countMSAOffices
                      }}>
                            รพ. M1 A S + สสจ./สนง.เขต
                          </TableHead>}

                        {showSummaryCols && <TableHead colSpan={3} className={`${stickyHeaderBase} text-center bg-rose-100 dark:bg-rose-900/30`} style={{
                        left: left.countM2F
                      }}>
                            รพ. M2 F1-F3
                          </TableHead>}
                            ...
                            {showSummaryCols && (() => {
                              ...
                              return <>
                                ...
                                <TableCell className={`${stickyCellBase} text-center font-medium bg-amber-50 dark:bg-amber-900/20`} style={{
                                  ...
                                  {score10 !== null ? (Number.isInteger(score10 * 0.7) ? String(score10 * 0.7) : (score10 * 0.7).toFixed(1)) : '-'}
                                </TableCell>
                              </>;
                            })()}
