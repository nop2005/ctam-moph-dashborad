...
-- Index สำหรับ filter assessments ตาม fiscal_year และ status
CREATE INDEX IF NOT EXISTS idx_assessments_fiscal_year_status 
  ON public.assessments(fiscal_year, status);

-- Index สำหรับหา latest assessment ของ hospital
CREATE INDEX IF NOT EXISTS idx_assessments_hospital_fiscal 
  ON public.assessments(hospital_id, fiscal_year DESC, assessment_period DESC) 
  WHERE hospital_id IS NOT NULL;

-- Index สำหรับหา latest assessment ของ health_office
CREATE INDEX IF NOT EXISTS idx_assessments_health_office_fiscal 
  ON public.assessments(health_office_id, fiscal_year DESC, assessment_period DESC) 
  WHERE health_office_id IS NOT NULL;

-- Index สำหรับ join hospitals กับ provinces
CREATE INDEX IF NOT EXISTS idx_hospitals_province_id 
  ON public.hospitals(province_id);

-- Index สำหรับ join health_offices กับ provinces และ regions
CREATE INDEX IF NOT EXISTS idx_health_offices_province_region 
  ON public.health_offices(province_id, health_region_id);

-- =====================================================
-- Phase 3: Create Cache Table for Public Reports
-- =====================================================

CREATE TABLE IF NOT EXISTS public.public_report_cache (
...
ALTER TABLE public.public_report_cache ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public read on public_report_cache" 
  ON public.public_report_cache 
  FOR SELECT 
  USING (true);

-- =====================================================
-- Phase 2: Rewrite get_public_report_summary function
-- Using CTEs instead of correlated subqueries
-- =====================================================
